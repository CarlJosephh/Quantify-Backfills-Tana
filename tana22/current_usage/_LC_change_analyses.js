...
function BFgrowth(image){
  //define current year system:id
  var id = ee.Number.parse(image.get('system:id'))
  
  //filter the backfill collection (importantly not increment) for previous year image
  var oldBF = ee.Image(Classi.filter(ee.Filter.eq('system:id', id.subtract(1).format())).first())
  
  //rename previous year band
  var dbands = oldBF.eq(0).rename('prevyrBFDist')
  
  //calculate a dstance kernel to later filter by current year BFs
  var distImg = dbands.distance(ee.Kernel.euclidean({radius: 3000, units: 'meters'}))
  
  //add the bands: current year backfill increment, previous year classification
  var bdImg = distImg.addBands([image, ee.Image(Classi.filter(ee.Filter.eq('system:id', id.subtract(1).format())).first()).rename('prevYearClassi')])
  
  return bdImg.set('system:id', id)
}



var incrementDist = maskRast.map(BFgrowth)
...
var FincrementDist = incrementDist.map(toFeature)

FincrementDist = FincrementDist.flatten()
//print(FincrementDist, 'FincrementDist')

Export.table.toDrive({collection: FincrementDist,
                        folder: 'GEEData',
                        description: 'oldBFDist_LCyr-1',
                        fileFormat: 'CSV',
                        selectors: (['Year' ,'prevyrBFDist', 'backfill', 'prevYearClassi'])
                        
})

//print(ee.Image(ClassiList.get(5)))
//Landcover change to BF 2017
var classi17 = BFgrowth(ee.Image(ClassiList.get(5)))
var ft17 = toFeature(classi17)

Export.table.toDrive({collection: ft17,
                      folder: 'GEEData',
                      description: 'LC_analyses_classi17',
                      fileFormat: 'CSV',
                      selectors: (['Year', 'prevyrBFDist', 'classification', 'prevYearClassi' ])
})

//Landcover change toBF 2018
var classi18 = BFgrowth(ee.Image(ClassiList.get(4)))
var ft18 = toFeature(classi18)

Export.table.toDrive({collection: ft18,
                      folder: 'GEEData',
                      description: 'LC_analyses_classi18',
                      fileFormat: 'CSV',
                      selectors: (['Year', 'prevyrBFDist', 'classification', 'prevYearClassi' ])
})
